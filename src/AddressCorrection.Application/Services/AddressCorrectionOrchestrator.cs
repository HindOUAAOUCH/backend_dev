...
/// <summary>
/// Orchestrateur pur de la correction d'adresses.
/// Responsabilité unique : valider l'entrée, déléguer au pipeline et retourner le résultat.
/// Toute la logique métier est encapsulée dans les étapes du pipeline.
/// </summary>
public class AddressCorrectionOrchestrator : IAddressCorrector
{
    ...
    public async Task<AddressResponse> CorrectAsync(AddressRequest request)
    {
        ...
        if (context.Error != null)
            ExceptionDispatchInfo.Capture(context.Error).Throw();
